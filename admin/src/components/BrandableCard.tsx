        ...
        {schoolSettings && (
            <div className="flex items-center gap-4 pb-4 border-b border-gray-200 dark:border-gray-700 mb-6">
                {schoolSettings.schoolLogoUrl && (
                    <img src={schoolSettings.schoolLogoUrl as string} alt="School Logo" className="w-10 h-10 rounded-lg" />
                )}
                <h2 className="text-lg font-bold text-gray-700 dark:text-gray-200">{schoolSettings.schoolName}</h2>
            </div>
        )}
        {children}
    </div>
