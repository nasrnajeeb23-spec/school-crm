...
>>>>>>> 35e46d4998a9afd69389675582106f2982ed28ae
  ...
  const beginEditDetails = (cls: Class) => {
    ...
    setEditTeacherId(String(cls.homeroomTeacherId || ''));
  };
  ...
  const classesFiltered = useMemo(() => {
    return classes
      ...
      .filter(c =>
        ...
        (c.homeroomTeacherName && c.homeroomTeacherName.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  }, [classes, stageFilter, gradeFilterUI, searchTerm]);

  const { sortedData: sortedClasses, handleSort, getSortIcon } = useSortableTable(classesFiltered, 'gradeLevel');
  ...
  const renderRow = (cls: Class) => {
    const isEditing = editingDetailsId === cls.id;
    return (
      <>
        <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
          {isEditing ? (
            <input value={editName} onChange={e => setEditName(e.target.value)} className="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 font-normal" />
          ) : (
            <span>{cls.gradeLevel} ({cls.section || 'أ'})</span>
          )}
        </td>
        <td className="px-6 py-4">{inferStageFromGrade(cls.gradeLevel)}</td>
        <td className="px-6 py-4">
          {isEditing ? (
            <select value={editTeacherId} onChange={e => setEditTeacherId(e.target.value)} className="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700">
              <option value="">اختر المعلم...</option>
              {teachers.map(t => (<option key={t.id} value={t.id}>{t.name}</option>))}
            </select>
          ) : (
            <span className="flex items-center">
              <UsersIcon className="h-4 w-4 ml-2 text-gray-400" />
        ...
        <td className="px-6 py-4">
          <span className="flex items-center">
            <StudentsIcon className="h-4 w-4 ml-2 text-gray-400" />
            {cls.studentCount}
          </span>
        </td>
        <td className="px-6 py-4">
          {isEditing ? (
            <input type="number" min={10} max={200} value={editCapacity} onChange={e => setEditCapacity(parseInt(e.target.value || '30'))} className="w-16 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700" />
          ) : (
        ...
        <td className="px-6 py-4">
          <div className="flex flex-wrap gap-1 max-w-xs">
            {cls.subjects.map((subject, index) => (
              <span key={index} className="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
                {subject}
              </span>
            ))}
            {cls.subjects.length === 0 && <span className="text-gray-400 text-xs text-center">—</span>}
          </div>
        </td>
        <td className="px-6 py-4 whitespace-nowrap">
          <div className="flex items-center gap-2">
            <button onClick={() => setEditingClass(cls)} className="font-medium text-teal-600 dark:text-teal-500 hover:underline">
              إدارة الطلاب
            </button>
            {isEditing ? (
              <>
                <button onClick={saveEditDetails} className="font-medium text-indigo-600 dark:text-indigo-500 hover:underline">حفظ</button>
                <button onClick={cancelEditDetails} className="font-medium text-gray-600 dark:text-gray-400 hover:underline">إلغاء</button>
              </>
            ) : (
              <button onClick={() => beginEditDetails(cls)} className="font-medium text-teal-700 dark:text-teal-400 hover:underline">تحرير</button>
            )}
            <button onClick={() => handleDeleteClass(cls)} className="font-medium text-red-600 dark:text-red-500 hover:underline">حذف</button>
            <button onClick={() => { setGenForClassId(cls.id); setGenDueDate(''); setGenIncludeBooks(true); setGenIncludeUniform(true); setGenIncludeActivities(true); setGenDiscounts([]); }} className="font-medium text-indigo-600 dark:text-indigo-500 hover:underline">فواتير</button>
          </div>
          {genForClassId === cls.id && (
            <div className="mt-2 p-3 border rounded-lg dark:border-gray-700 bg-gray-50 dark:bg-gray-800 absolute z-50 w-72 shadow-lg -translate-x-1/2 left-1/2">
              <h5 className="font-bold mb-2">توليد فواتير</h5>
              <div className="space-y-2 text-sm">
                <input type="date" value={genDueDate} onChange={e => setGenDueDate(e.target.value)} className="w-full px-2 py-1 border rounded dark:bg-gray-700" />
                <div className="flex flex-wrap gap-2">
                  <label className="flex items-center gap-1"><input type="checkbox" checked={genIncludeBooks} onChange={e => setGenIncludeBooks(e.target.checked)} /> <span>كتب</span></label>
                  <label className="flex items-center gap-1"><input type="checkbox" checked={genIncludeUniform} onChange={e => setGenIncludeUniform(e.target.checked)} /> <span>زي</span></label>
                  <label className="flex items-center gap-1"><input type="checkbox" checked={genIncludeActivities} onChange={e => setGenIncludeActivities(e.target.checked)} /> <span>أنشطة</span></label>
                </div>
                <div className="flex gap-2">
                  <button onClick={async () => { if (!genDueDate) { addToast('حدد تاريخ الاستحقاق.', 'error'); return; } try { const studs = await api.getClassStudents(cls.id); const ids = studs.map(s => s.id); const res = await api.generateInvoicesFromFees(schoolId, { studentIds: ids, dueDate: genDueDate, include: { books: genIncludeBooks, uniform: genIncludeUniform, activities: genIncludeActivities }, defaultDiscounts: genDiscounts }); addToast(`تم إنشاء ${res.createdCount} فاتورة.`, 'success'); setGenForClassId(''); } catch { addToast('فشل إنشاء الفواتير.', 'error'); } }} className="flex-1 px-2 py-1 bg-teal-600 text-white rounded hover:bg-teal-700">تأكيد</button>
                  <button onClick={() => setGenForClassId('')} className="flex-1 px-2 py-1 bg-gray-300 rounded hover:bg-gray-400 dark:bg-gray-700 dark:text-gray-200">إلغاء</button>
                </div>
              </div>
            </div>
          )}
  ...
  const renderCard = (cls: Class) => {
    return (
      <div key={cls.id}>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap justify-end gap-2">
          <button onClick={() => setEditingClass(cls)} className="font-medium text-teal-600 dark:text-teal-500 hover:underline text-sm">
            إدارة الطلاب
          </button>
          {editingDetailsId === cls.id ? (
            <>
              <button onClick={saveEditDetails} className="font-medium text-indigo-600 dark:text-indigo-500 hover:underline text-sm">حفظ</button>
          ...
          {editingDetailsId === cls.id ? null : (
            <button onClick={() => beginEditDetails(cls)} className="font-medium text-teal-700 dark:text-teal-400 hover:underline text-sm">تحرير</button>
          )}
          <button onClick={() => handleDeleteClass(cls)} className="font-medium text-red-600 dark:text-red-500 hover:underline text-sm">حذف</button>
          <button onClick={() => { setGenForClassId(cls.id); setGenDueDate(''); setGenIncludeBooks(true); setGenIncludeUniform(true); setGenIncludeActivities(true); setGenDiscounts([]); }} className="font-medium text-teal-600 dark:text-teal-500 hover:underline text-sm">فواتير</button>
        </div>
        {genForClassId === cls.id && (
          ...
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
              <div className="flex flex-wrap gap-4 items-center mb-4">
                ...
                />
                <select value={stageFilter} onChange={e => { setStageFilter(e.target.value); setGradeFilterUI(''); }} className="w-full md:w-40 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500">
                  <option value="">اختر المرحلة...</option>
                  {availableStages.map(s => (<option key={s} value={s}>{s}</option>))}
                </select>
                <select value={gradeFilterUI} onChange={e => setGradeFilterUI(e.target.value)} className="w-full md:w-40 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500" disabled={!stageFilter}>
              ...
              <ResponsiveTable
                headers={['اسم الفصل', 'المرحلة', 'المعلم المسؤول', 'الطلاب', 'السعة', 'المواد', 'إجراءات']}
                data={paginatedClasses}
                renderRow={renderRow}
                renderCard={renderCard}
                keyExtractor={(cls) => String(cls.id)}
              />
              <Pagination
