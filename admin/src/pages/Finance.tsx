...
import { Invoice, InvoiceStatus, PaymentData, NewInvoiceData, Expense, NewExpenseData, ExpenseCategory, SchoolSettings, Teacher, User } from '../types';
...
const Finance: React.FC<FinanceProps> = ({ schoolId, schoolSettings }) => {
    ...
    const fetchData = () => {
        ...
        ]).then(([invoicesData, expensesData, structuresData, staffData, teachersData, slipsData]) => {
    ...
    const tabs = [ { id: 'overview', label: 'نظرة عامة', icon: RevenueIcon }, { id: 'invoices', label: 'الفواتير (الإيرادات)', icon: FileIcon }, { id: 'expenses', label: 'المصروفات', icon: ExpenseIcon }, { id: 'payroll', label: 'الرواتب', icon: NetProfitIcon } ];
