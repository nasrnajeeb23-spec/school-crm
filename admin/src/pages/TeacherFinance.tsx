        ...
        <div className="mt-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <StatsCard 
                    ...
                    value={latestSlip ? formatCurrency(latestSlip.netSalary) : 'N/A'}
                    description={`لشهر ${latestSlip?.month} ${latestSlip?.year}`}
                />
                <StatsCard 
                    icon={FinanceIcon}
                    title="الراتب الإجمالي"
                    value={latestSlip ? formatCurrency(latestSlip.grossSalary) : 'N/A'}
                            ...
                            <div key={slip.id} className="border border-gray-200 dark:border-gray-700 rounded-lg">
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className="font-semibold text-gray-800 dark:text-white">{formatCurrency(slip.netSalary)}</span>
                                    ...
                                    <div className="px-4 pb-4 border-t border-gray-200 dark:border-gray-700">
                                        <div className="space-y-2 mt-4">
                                            <div className="flex justify-between py-2">
                                                ...
                                                <p className="font-semibold dark:text-white">{formatCurrency(slip.grossSalary)}</p>
                                            </div>
                                            ...
                                            <div className="flex justify-between pt-3 mt-2 border-t-2 border-gray-200 dark:border-gray-600">
                                                ...
                                                <p className="font-bold text-lg text-teal-600 dark:text-teal-400">{formatCurrency(slip.netSalary)}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
